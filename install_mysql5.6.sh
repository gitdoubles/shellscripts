...
MYSQL_START_SCRIPTNAME=mysql5.6
ERROR_EXIT=65
...
{    
    ...
    # if the basedir ${MYSQLDB_HOME} is exsits.....
    if [ -d "${MYSQLDB_HOME}" ]
    then
        echo "The basedir ${MYSQLDB_HOME} is exsits"
        ...
        then
            ...
            MYSQLDB_HOME=/usr/local/$MYSQL_START_SCRIPTNAME
            mv ${MYSQL_VERSION} ${MYSQLDB_HOME}
        fi
    else
        mv ${MYSQL_VERSION} ${MYSQLDB_HOME}
    fi
    # if the datadir ${MYSQLDB_HOME} is exsits.....
    ...
    chown -R USER_NAME:GROUP_NAME ${MYSQLDB_HOME}
...
Install_mysql()
{
    ...
    ${MYSQLDB_HOME}/scripts/mysql_install_db --user=$USER_NAME --datadir=${MYSQLDB_DATA_HOME} --basedir=${MYSQLDB_HOME}
    if(( $? == 0 ))
    then
...
pid-file=${MYSQLDB_DATA_HOME}/mysqld.pid" >./my.cnf
    ...
    cp ${MYSQLDB_HOME}/my.cnf /etc/my.cnf

    # add the bin to the path
    echo "add the $MYSQLDB_HOME/bin to the path!"
    cat >> /etc/profile <<EOF  
...
Start_mysql()
{
    ...
    cp ${MYSQLDB_HOME}/support-files/mysql.server /etc/init.d/$MYSQL_START_SCRIPTNAME
...
Create_user()
{
    ...
    ${MYSQLDB_HOME}/bin/mysql $MYSQL_CONNECT_PARAMETER  <<EOF 
